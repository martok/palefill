...
const _definitions = [
    {
        ...
        fix: ["gh-compat", "qmicrotask", "sm-gh-extra", "sm-cookie", "gh-worker-csp"]
    }
]
...
class PolyfillService {
    ...
    loadDefinitions() {
        for (const defn of _definitions) {
            const key = Array.sort(defn.fix).join('+')
            for (const host of defn.selector) {
                this.selectors[host] = key;
            }
            if (!this.fixes.hasOwnProperty(key)) {
                this.fixes[key] = {
                    a: [...defn.fix]
                }
            }
    ...
    getFixes(URI) {
        // TODO: Merge fixes from all expression matching defs
        const key = this.selectors[URI.host];
        if (!key) return null;
        return this.fixes[key];
    }
...
class HTTPObserver {
    ...
    observe(subject, topic, data) {
        ...
        const fixes = service.getFixes(subject.URI);
        switch (topic) {
