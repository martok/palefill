...
const _definitions = [
    {
        ...
        fill: ["gh-compat", "qmicrotask", "sm-gh-extra", "sm-cookie"]
    }
]
...
class HTTPObserver {
    ...
    isLoadedDocument(subject) {
        return subject.responseStatus == 200 &&
    ...
    isHTMLDocument(subject) {
        return subject.getResponseHeader("Content-Type").indexOf("text/html") != -1;
    }
        ...
        switch (topic) {
            ...
            case "http-on-examine-cached-response":
                if (subject.responseStatus == 200 || subject.responseStatus == 304) {
                    try {
                        if (this.isLoadedDocument(subject) && this.isHTMLDocument(subject)) {
