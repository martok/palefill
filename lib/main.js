...
const {
    CSP, encodeHTMLAttribute, Unicode, sha256,
...
function evaluateFix(fix, script, csp, contentReplace) {
    switch(fix) {
        ...
        case "std-AudioWorkletNode":
            ...
            script.push({"src": "https://unpkg.com/audioworklet-polyfill@1.1.2/dist/audioworklet-polyfill.js",
        ...
        case "std-NavigatorClipboard":
            csp["script-src"].push("unpkg.com");
            script.push({"src": "https://unpkg.com/clipboard-polyfill@4.1.1/dist/es5/overwrite-globals/clipboard-polyfill.overwrite-globals.es5.js",
                          "integrity": "sha384-LNw3jpbT90KNXi+C336S12H/Lnw/ln+ndZLKbI8FOhqXyOn/3eQAlUUOs/xZySEb"});
            break;
        ...
        case "vt-nomodule":
            ...
            break;
        case "wordpress-logicalassign":
            contentReplace.push([`e.performanceMarks??=[]`, `e.performanceMarks??(e.performanceMarks=[])`]);
            contentReplace.push([`(o[o.length-1].steps??=[])`, `(o[o.length-1].steps??(o[o.length-1].steps=[]))`]);
            contentReplace.push([`s.duration??=t-s.startTime`, `s.duration??(s.duration=t-s.startTime)`]);
            break;
...
function supersededFixes(service) {
    ...
    if (service.isClipboardSupported) {
        superseded.add("std-NavigatorClipboard");
    }
    return superseded;
}
...
class MergedFix {
    ...
    ensureCompiled() {
        ...
        // do a first simplification step 
        this.csp = CSP.dictSimplify(csp);
        this.compiled = true;
    }
...
class PolyfillService {
    constructor() {
        ...
        this.isClipboardSupported = false;
    ...
    _checkWindowFeatures(window) {
        ...
        this.isClipboardSupported = typeof window.navigator !== "undefined" &&
    ...
    modifyContentSecurityPolicy(cspHeader, activeFixes) {
        activeFixes.ensureCompiled();
        // no change required
        if (!Object.getOwnPropertyNames(activeFixes.csp).length || !cspHeader) {
            return cspHeader;
        }
        // parse map
        const policies = CSP.headerToDict(cspHeader);
        // merge new rules into existing
        CSP.dictMap(activeFixes.csp, (dir, lst) => {
            const existing = policies.hasOwnProperty(dir);
            const target = existing ? policies[dir] : [];
            if ("script-src" === dir) {
                // special case: if we introduce nonce- or hash- (lvl2) sources when the original only uses lvl1, unsafe-* get disabled
                if (!target.length) {
                    // didn't have any restrictions originally, don't introduce any
                    return;
                }
                const usedUnsafe = target.some(e => e == "'unsafe-inline'");
                const usedSafe = target.some(e => e.startsWith("'sha256-") || e.startsWith("'nonce-"));
                // if original used both hash/nonce and unsafe-inline, that unsafe is only a fallback for older browsers.
                // in that case, follow originals intention and declare our scripts fully.
                // otherwise, we must skip them to not break unsafe-inline.
                if (usedUnsafe && !usedSafe) {
                    // for external scripts, don't set the nonce (avoid lvl2), host name is also set by evaluateFix
                    lst = lst.filter(e => !e.startsWith("'sha256-") && !e.startsWith("'nonce-"));
                    if (!lst.length) {
            ...
            // merge and deduplicate new values into old
        ...
        const newHeader = CSP.dictToHeader(policies);
        return newHeader;
    }

    getScriptInsertPoint(data, lookForMeta = false) {
        const pHead = data.indexOf("<head");
        if (pHead < 0)
            return false;
        if (lookForMeta) {
            // insert after charset tag if this is expected
            const pCharset = data.indexOf("<meta charset=", pHead+5);
            if (pCharset >= 0) {
                const pPoint = data.indexOf(">", pCharset+16);
                if (pPoint < 0)
                    return false;
                return pPoint + 1;
            }
        }
        // just insert directly after head
        const pPoint = data.indexOf(">", pHead+5);
        if (pPoint < 0)
            return false;
        return pPoint + 1;
    }
    ...
    modifyRequestData(data, activeFixes) {
        ...
        if (activeFixes.scripts) {
            const splicePos = this.getScriptInsertPoint(data.slice(0, 1024), true) ||
                              this.getScriptInsertPoint(data);
            if (splicePos >= 0) {
                data = data.slice(0, splicePos) + activeFixes.scripts + data.slice(splicePos);
            }
        }
        return data;
    }
