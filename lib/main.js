...
class MergedFix {
    ...
    ensureCompiled() {
        ...
        let inline = "";
        let nonce = sha256("Palefill" + Math.random() + (new Date()).toLocaleFormat()); // not cryptographically secure, only needs to be reasonably unguessable
...
class HTTPObserver {
    ...
    isScript(subject) {
        if (subject.loadInfo.externalContentPolicyType == nsIContentPolicy.TYPE_SCRIPT)
            return true;
    ...
    shouldProcessResponse(subject) {
        // regular responses
        if ([200, 304].includes(subject.responseStatus))
           return true;
        // errors that usually have error pages, but *only* if this is an error page
        if (400 <= subject.responseStatus && subject.responseStatus <= 499 && this.isHTMLDocument(subject))
            return true;
    ...
    observe(subject, topic, data) {
        ...
        // full test
        const fixes = gService.getFixes(subject.URI, this.isScript(subject)?nsIContentPolicy.TYPE_SCRIPT:subject.loadInfo.externalContentPolicyType);
        if (fixes === null) {
            return;
        }
        switch (topic) {
            ...
            case "http-on-examine-cached-response":
                if (this.shouldProcessResponse(subject)) {
                    try {
