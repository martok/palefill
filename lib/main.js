...
class RuleEngine {
        ...
        for (const defn of defs) {
            ...
            for (const selstr of defn.selector) {
                ...
                try {
                    ...
                    print("Error in expression: ", e.message, " at ", selstr);
                    continue;
                }
            }
        }
    ...
    addRulesFromString(script) {
        ...
        for (const line of lines) {
            ...
            if (parts.length!== 2) {
                print("Error in expression: missing separator at ", line);
                continue;
            }
            const fixset = new Set(parts[1].split(",").filter(s => !!s.length));
            if (!fixset.size) {
                print("Error in expression: no fixes at ", line);
                continue;
            }
            try {
                const selector = RuleSelector.parse(parts[0]);
                if (selector) {
                    this.enabledDomains.add(selector.domain);
                    this.selectors.set(selector, fixset);
                    accepted++;
                ...
                print("Error in expression: ", e.message, " at ", parts[0]);
                continue;
            }
        }
        return [accepted, processed];
...
class PolyfillService {
    constructor() {
        ...
        settings.onOptionsDisplayed.on(this._optionsDisplayed, this);
    }

    destroy() {
        settings.onPrefChanged.remove(this._prefChanged, this);
        settings.onOptionsDisplayed.remove(this._optionsDisplayed, this);
    }
    ...
    _optionsDisplayed(xul) {
        ...
        btnApply.addEventListener("click", () => {
            ...
            const [a, p] = tmpRules.addRulesFromString(newText);
            if (a == p) {
