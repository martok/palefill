...
const _definitions = [
    {
        ...
        fix: ["std-PerformanceObserver", "gh-temp-oldindex2", "gh-compat", "qmicrotask", "sm-gh-extra", "sm-cookie"]
    },
    {
...
class PolyfillService {
    ...
    ensureCompiledFixes(activeFixes) {
        ...
        for (const item of activeFixes.a) {
            switch(item) {
                case "$script":             // marker: apply also to script content
                    break;
                case "gh-compat":
                    scripts.push(pf.Element_attachShadow);
                ...
                case "qmicrotask":
                    ...
                    break;
                case "customElements":
                    scripts.push(pf.Window_customElements);
                    break;
                case "std-ShadowDOM":
                    csp["script-src"].push("unpkg.com");
                    scripts.push({"src": "https://unpkg.com/@webcomponents/shadydom@1.9.0/shadydom.min.js",
                                  "integrity": "sha384-PFKLoiJYZCqKOdOUvo0Z18Ofro9s8QJkXaagRBHR4z8WMAyGeyp0ZWmziwHoeI7K"});
                    break;
                case "std-PerformanceObserver":
                    csp["script-src"].push("unpkg.com");
                    scripts.push({"src": "https://unpkg.com/@fastly/performance-observer-polyfill@2.0.0/polyfill/index.js",
                ...
                case "dhl-optchain":
                    ...
                    break;
                case "gl-scripts":
                    contentReplace.push([`\\p{Emoji}`, pf.Regex_EmojiCharacterClass]);
                    contentReplace.push([`(?<iid>`, `(`]);
                    contentReplace.push([`.groups.iid`, `[1]`]);
                    // https://gitlab.com/gitlab-org/gitlab/-/merge_requests/79161
                    contentReplace.push([/\(\?<(indent|leader|isOl|isUl|content)>/g, "(<$1>"]);
                    contentReplace.push([/(const{indent:.,content:.,leader:.}=)o.groups;/, "$1{};"]);
            }
        }
        ...
        for (const script of scripts) {
            if (typeof script === "string") {
                ...
                const attribs = {"crossorigin": "anonymous", "type": "application/javascript"};
                const scr = ['<script'];
            ...
            case "http-on-examine-cached-response":
                if ([200, 304].includes(subject.responseStatus)) {
                    try {
                        ...
                        } catch (e) { /*eat NS_ERROR_NOT_AVAILABLE exception if no CSP is set */ }
